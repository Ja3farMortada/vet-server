...
class Reservation {
	...
	static async getAll() {
		let query = `SELECT R.*, A.name AS title, A.phone FROM reservations R INNER JOIN accounts A ON R.customer_id_fk = A.account_id WHERE R.is_deleted = 0`;
	...
	static async getById(id) {
		let query = `SELECT R.*, A.name AS title, A.phone FROM reservations R INNER JOIN accounts A ON R.customer_id_fk = A.account_id WHERE R.is_deleted = 0 AND id = ?`;
